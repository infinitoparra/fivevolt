...
uint8_t SdVolume::init(Sd2Card* dev, uint8_t part) 
{
  ...
  if (part) 
  {
    ...
    /*
    SerialDebug.print("boot: ");
	  SerialDebug.println(p->boot, 16);
	  SerialDebug.print("totalSectors: ");
	  SerialDebug.println(p->totalSectors, 16);
	  SerialDebug.print("firstSector: ");
	  SerialDebug.println(p->firstSector, 16);
	  */
  ...
  /*
  SerialDebug.print("volumeStartBlock: ");
  SerialDebug.println(volumeStartBlock, 16);
  */
...
int16_t SdFile::read(RingBuffer* buf, uint16_t nbyte) {
  // error if not open or write only
  if (!isOpen() || !(flags_ & O_READ)) return -1;
  ...
  while (toRead > 0) {
    ...
    if ((unbufferedRead() || n == 512) &&
      block != SdVolume::cacheBlockNumber_) {
      if (!vol_->readData(block, offset, n, buf)) return -1;
    } else {
      // read block to cache and copy data to caller
      if (!SdVolume::cacheRawBlock(block, SdVolume::CACHE_FOR_READ)) return -1;
      uint8_t* src = SdVolume::cacheBuffer_.data + offset;
      uint8_t* end = src + n;
      while (src != end) buf->insert(*src++);
    }
    curPosition_ += n;
    toRead -= n;
  }
 ...
 */
uint8_t Sd2Card::init(HardwareSPI *s, uint8 cs) 
{
...
uint8_t Sd2Card::readData(uint32_t block,
        uint16_t offset, uint16_t count, RingBuffer* buf) {
  if (count == 0) return true;
  if ((count + offset) > 512) 
  {
    goto fail;
...
uint8_t Sd2Card::writeBlock(uint32_t blockNumber, const uint8_t* src) 
{
  ...
  {
	SerialDebug.println("Error: CMD42");
